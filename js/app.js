/* ============================================
   EXPENSETRACKER — Main Application  v3
   Backend : Supabase (with IndexedDB fallback)
   ============================================ */
'use strict';

/* ════════════════════════════════
   DETECT BACKEND
   If Supabase is configured, use it. Else fall back to local IndexedDB.
   ════════════════════════════════ */
const USE_SUPABASE = (typeof SB !== 'undefined') &&
  !window.location.href.includes('YOUR_SUPABASE') &&
  (typeof SUPABASE_URL === 'undefined' || SUPABASE_URL !== 'YOUR_SUPABASE_PROJECT_URL');

/* Unified data API — resolved at boot */
let Api = null;
...
const CATEGORIES = {
  ...
  travel:        { label:'Travel',    emoji:'✈️', color:'#0891b2' },
  ...
  training:      { label:'Training',  emoji:'📚', color:'#0e7490' },
  other:         { label:'Other',     emoji:'💼', color:'#64748b' }
};

const AVATAR_COLORS = [
  '#0891B2','#059669','#DC2626','#7C3AED',
  '#D97706','#0284C7','#DB2777','#16A34A',
  '#EA580C','#0E7490','#4F46E5','#65A30D'
];
...
function firstDayOfMonth(y, m) { return `${y}-${String(m+1).padStart(2,'0')}-01`; }
...
function avatarColor(id) {
  const key = typeof id === 'number' ? id - 1 : [...String(id)].reduce((a,c) => a + c.charCodeAt(0), 0);
...
function showOverlay() { document.getElementById('loadingOverlay').classList.add('active'); }
function hideOverlay() { document.getElementById('loadingOverlay').classList.remove('active'); }

function animateCounter(el, target, format = v => String(Math.round(v)), duration = 500) {
...
function skeletonReceiptList(container, count = 4) {
  container.innerHTML = Array.from({length:count}, () => `
    <div class="receipt-card" style="pointer-events:none">
      <div class="receipt-thumb skeleton"></div>
      <div class="receipt-info" style="gap:6px">
        <div class="skeleton skeleton-line w-7"></div>
        <div class="skeleton skeleton-line w-5"></div>
        <div class="skeleton skeleton-line w-3"></div>
      </div>
      <div style="text-align:right;flex-shrink:0;width:64px">
        <div class="skeleton skeleton-line w-f"></div>
        <div class="skeleton skeleton-line w-5" style="margin-left:auto"></div>
      </div>
    </div>`).join('');
}
...
async function init() {
  ...
  if (USE_SUPABASE) {
    ...
    try {
      ...
      SB.onAuthChange(async (session) => {
        if (session && !State.currentUser) await loginWithSession(session);
        else if (!session && State.currentUser) { State.currentUser = null; showAuthScreen(); }
      });
    } catch (err) {
      console.error('Supabase init error:', err);
      hideSplash();
      showToast('Could not connect to Supabase — check your config', 'error');
      showAuthScreen();
    }
  } else {
    /* ── Fallback: IndexedDB ── */
    ...
    try {
      ...
      if (savedId && State.allUsers.find(u => u.id === savedId)) {
        await loginUserLocal(savedId, true);
      } else {
        _showLoginScreen();
      }
    } catch (err) {
...
async function loginWithSession(session) {
  ...
  try {
    ...
    if (!profile) {
      /* First sign-up — profile may not exist yet */
      await SB.updateUser({
        id: session.user.id,
        name: session.user.user_metadata?.name || session.user.email.split('@')[0],
        role: 'user', currency: 'USD', monthlyBudget: 2000, darkMode: false, adminPin: ''
      });
      State.currentUser = await SB.getProfile(session.user.id);
    } else {
    ...
    Object.assign(State.settings, {
      ...
      monthlyBudget: u.monthlyBudget || 2000,
      darkMode:      u.darkMode      || false,
      adminPin:      u.adminPin      || ''
    });
    applyTheme();

    State.allUsers    = await SB.getAllUsers().catch(() => []);
    State.allReceipts = await SB.getAll().catch(() => []);
    State.receipts = canSeeTeam()
      ? State.allReceipts
      : State.allReceipts.filter(r => r.userId === session.user.id);
    ...
    renderDashboard();
    showToast(`Welcome back, ${State.currentUser.name.split(' ')[0] || 'User'}!`, 'success');
  } catch (err) {
...
function setupAuthListeners() {
  ...
  document.querySelectorAll('.auth-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      document.getElementById('authPanelSignin').classList.toggle('hidden', tab.dataset.auth !== 'signin');
      document.getElementById('authPanelSignup').classList.toggle('hidden', tab.dataset.auth !== 'signup');
    });
  });
  ...
  document.getElementById('btnSignIn').addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('btnSignIn').innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg> Sign In';
    }
  });
  ...
  document.getElementById('btnSignUp').addEventListener('click', async () => {
    ...
    document.getElementById('btnSignUp').textContent = 'Creating account…';
    try {
      const session = await SB.signUp(email, password, name);
      if (session) {
        await loginWithSession(session);
      } else {
        errEl.textContent = 'Account created! Please check your email to confirm, then sign in.';
        errEl.classList.add('visible');
        document.getElementById('btnSignUp').innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Create Account';
      }
    } catch (err) {
...
function renderUserGrid() {
  ...
  grid.innerHTML = State.allUsers.map(u => {
    ...
    return `<div class="user-login-card" onclick="loginUserLocal(${u.id})">
...
async function loginUserLocal(userId, skipAnim = false) {
  ...
  renderDashboard();
  hideLoginScreen();
  if (!skipAnim) showToast(`Welcome, ${user.name.split(' ')[0]}!`, 'success');
}

function showLoginScreen_public() {
  if (USE_SUPABASE) {
    SB.signOut().then(() => { State.currentUser = null; showAuthScreen(); }).catch(console.error);
  } else {
...
function loadSettings() {
  try {
    const s = localStorage.getItem('ef_settings');
...
function populateSettingsForm() {
  ...
  document.getElementById('sEmail').value        = u.email      || u.id || '';
  ...
  document.getElementById('sManagerEmail').value = s.managerEmail || u.managerEmail || '';
  ...
  document.getElementById('settingsAvatarEmail').textContent = u.email || (USE_SUPABASE ? 'Linked via Supabase' : 'Add your email');
...
function verifyAdminPin() {
  return new Promise(resolve => {
    ...
    function close(r) { modal.classList.add('hidden'); confirmB.removeEventListener('click', onConfirm); cancelB.removeEventListener('click', onCancel); form.removeEventListener('submit', onConfirm); resolve(r); }
    function onConfirm() { if (input.value === State.settings.adminPin) { close(true); } else { showToast('Incorrect PIN', 'error'); input.value=''; input.focus(); } }
...
function setupNavigation() {
  document.querySelectorAll('.nav-item').forEach(btn => btn.replaceWith(btn.cloneNode(true)));
...
function renderDashboard() {
  ...
  document.getElementById('greetingName').textContent = u?.name?.split(' ')[0] || 'User';
  ...
  /* Animate counters */
  animateCounter(document.getElementById('dashPending'),  pending);
  animateCounter(document.getElementById('dashApproved'), approved);
  animateCounter(document.getElementById('dashTotal'), allTotal, v => formatCurrency(v, currency));
...
function renderRecentReceipts() {
  ...
  container.querySelectorAll('.receipt-card').forEach(card =>
    card.addEventListener('click', () => openReceiptModal(card.dataset.id))
  );
}
...
function renderReceiptsList() {
  ...
  let filtered = State.receipts.filter(r => {
    ...
    if (userFilter !== 'all' && userFilter !== 'self') {
      if (r.userId !== userFilter) return false;
    }
    return true;
  });
  ...
  document.getElementById('receiptsCount').textContent =
    `${filtered.length} receipt${filtered.length !== 1 ? 's' : ''} · ${formatCurrency(total, State.settings.currency)}`;
...
function populateFilterUsers() {
  ...
  State.allUsers.forEach(u => {
    const opt1 = document.createElement('option'); opt1.value = u.id; opt1.textContent = u.name; select.appendChild(opt1);
    if (teamSel) { const opt2 = document.createElement('option'); opt2.value = u.id; opt2.textContent = u.name; teamSel.appendChild(opt2); }
  });
}
...
async function saveReceipt(e) {
  ...
  try {
    if (editId) {
      ...
      const id = await Api.add(receipt);
      receipt.id = id;
...
async function renderTeamReport() {
  ...
  /* Default date range to current month if not set */
  if (!fromEl.value) fromEl.value = firstDayOfMonth(year, month);
  if (!toEl.value)   toEl.value   = lastDayOfMonth(year, month);

  const from = fromEl.value;
  const to   = toEl.value;
  const filterUid = teamSel.value === 'all' ? null : teamSel.value;

  let monthReceipts;
  if (USE_SUPABASE && from && to) {
    try {
      monthReceipts = await SB.getByDateRange(from, to, filterUid);
    } catch {
      monthReceipts = State.allReceipts.filter(r => r.date >= from && r.date <= to && (!filterUid || r.userId === filterUid));
    }
  } else {
    monthReceipts = State.allReceipts.filter(r => {
      const d = new Date(r.date + 'T00:00:00');
      return d.getMonth() === month && d.getFullYear() === year &&
             (!filterUid || r.userId === filterUid);
    });
  }
...
async function generatePDF(receiptsOverride, titleOverride, returnBlob = false) {
  ...
  /* Header band */
  doc.setFillColor(9,9,11); doc.rect(0,0,pageW,40,'F');
  doc.setFillColor(14,116,144); doc.rect(pageW-55,0,55,40,'F');
  ...
  boxes.forEach((box, i) => {
    ...
    doc.setTextColor(15,23,42); doc.setFont('helvetica','bold'); doc.setFontSize(i===0?9:11);
  ...
  if (catEntries.length) {
    ...
    catEntries.forEach(([cat,amt],i) => {
      ...
      doc.setFillColor(14,116,144);  doc.roundedRect(bx, y-3.5, bf,  3, 1,1,'F');
    });
  }
  ...
  if (receipts.length) {
    ...
    doc.autoTable({
      ...
      footStyles:{ fillColor:[248,250,252], textColor:[9,9,11], fontStyle:'bold' },
  ...
  if (withImages.length) {
    doc.addPage();
    doc.setFillColor(9,9,11); doc.rect(0,0,pageW,pageH,'F');
    doc.setFillColor(14,116,144); doc.rect(0,pageH-2,pageW,2,'F');
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(10); doc.setTextColor(34,211,238);
    ...
    for (const r of withImages) {
      ...
      doc.setFillColor(14,116,144); doc.rect(0,23.5,pageW,0.5,'F');
      doc.setTextColor(9,9,11); doc.setFont('helvetica','bold'); doc.setFontSize(13);
      ...
      if (r.description) { doc.setFontSize(7.5); doc.setTextColor(100,116,139); doc.text(r.description.slice(0,120), 14, 21.5); }
      try {
        const {w:natW, h:natH} = await imageNaturalSize(r.imageData);
        const maxW = pageW-28, maxH = pageH-24-14;
        let imgW=maxW, imgH=maxW*(natH/natW||1);
        if (imgH > maxH) { imgH=maxH; imgW=maxH*(natW/natH||1); }
        doc.addImage(r.imageData, fmt, (pageW-imgW)/2, 28, imgW, imgH, undefined, 'FAST');
      } catch {
...
async function generateTeamPDF() {
  ...
  let receipts;
  if (USE_SUPABASE && from && to) {
    receipts = await SB.getByDateRange(from, to, filterUid).catch(() => []);
  } else {
    receipts = State.allReceipts.filter(r => {
      const d = new Date(r.date + 'T00:00:00');
      return d.getMonth() === month && d.getFullYear() === year && (!filterUid || r.userId === filterUid);
    });
  }
...
async function sendReportEmail() {
  ...
  showOverlay();
  let blob;
  try { blob = await generatePDF(receipts, null, true); }
  catch { hideOverlay(); showToast('Could not generate PDF', 'error'); return; }
  hideOverlay();
  if (!blob) return;

  const file = new File([blob], fname, { type:'application/pdf' });
  if (navigator.canShare && navigator.canShare({ files:[file] })) {
    try { await navigator.share({ title:subject, text:bodyText, files:[file] }); showToast('Report shared!', 'success'); return; }
  ...
  setTimeout(() => {
    window.location.href = `mailto:${s.managerEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyText + '\n\n[Attach the downloaded PDF]')}`;
  }, 600);
...
function renderUserManagementList() {
  ...
  if (!State.allUsers.length) { container.innerHTML = '<p style="color:var(--text-3);font-size:.85rem;padding:14px 16px">No team members found</p>'; return; }
  container.innerHTML = State.allUsers.map(u => {
    ...
    return `<div class="user-row">
      ...
      <div class="user-row-info">
        ...
        <div class="user-row-email">${roleLbl}${u.department?' · '+escapeHtml(u.department):''}</div>
      </div>
...
async function saveUserForm() {
  ...
  if (isAdmin() && elevatedRole && !firstUser) {
    if (State.settings.adminPin) {
      ...
      if (!ok) return;
    } else { showToast('Tip: set an Admin PIN in Settings → Security', 'info'); }
  }
  ...
  try {
    if (editId) {
      ...
      if (idx >= 0) State.allUsers[idx] = { ...State.allUsers[idx], ...userData };
      if (State.currentUser?.id === userData.id) { State.currentUser = { ...State.currentUser, ...userData }; updateHeaderUser(); populateSettingsForm(); }
      showToast('Profile updated', 'success');
    } else {
      const id = await Api.addUser(userData);
      userData.id = id;
    ...
    console.error(err); showToast('Failed to save profile', 'error');
  }
}
...
function setupEventListeners() {
  ...
  clone('btnCancelCamera').addEventListener('click', () => { stopCameraStream(); document.getElementById('capturePlaceholder').classList.remove('hidden'); });
  ...
  document.getElementById('fLocation').addEventListener('input', e => {
    ...
    document.getElementById('iataTag').textContent = val.length >= 2 ? val : '';
  });
  ...
  ['filterCategory','filterMonth','filterStatus','filterUser'].forEach(id => {
    const el = document.getElementById(id); if (el) el.addEventListener('change', () => renderReceiptsList());
  });
  document.getElementById('clearFilters').addEventListener('click', () => {
    ['filterCategory','filterMonth','filterStatus','filterUser'].forEach(id => { const el=document.getElementById(id); if(el) el.value='all'; });
    renderReceiptsList();
  });
  ...
  clone('modalEdit').addEventListener('click', async () => {
    const id = State.modalReceiptId; closeReceiptModal();
    const receipt = await Api.getById(id);
  ...
  document.getElementById('btnApplyRange')?.addEventListener('click', () => renderTeamReport());

  /* Report month nav */
  clone('reportPrevMonth').addEventListener('click', () => { State.reportMonth--; if(State.reportMonth<0){State.reportMonth=11;State.reportYear--;} renderReport(); });
  clone('reportNextMonth').addEventListener('click', () => { State.reportMonth++; if(State.reportMonth>11){State.reportMonth=0;State.reportYear++;} renderReport(); });
  clone('btnGeneratePDF').addEventListener('click', () => { showOverlay(); generatePDF().finally(hideOverlay); });
  clone('btnSendReport').addEventListener('click', sendReportEmail);
  document.getElementById('btnTeamPDF')?.addEventListener('click', () => { showOverlay(); generateTeamPDF().finally(hideOverlay); });
  ...
  clone('btnSaveSettings').addEventListener('click', async () => {
    ...
    const updated = {
      ...
      monthlyBudget:parseFloat(document.getElementById('sMonthlyBudget').value)||2000,
      currency:     document.getElementById('sCurrency').value,
      darkMode:     document.getElementById('sDarkMode').checked,
      adminPin:     isAdmin() ? document.getElementById('sAdminPin').value.trim() : (State.settings.adminPin||'')
    };
    try {
  ...
  document.getElementById('sDarkMode').addEventListener('change', e => { State.settings.darkMode=e.target.checked; applyTheme(); });

  /* Team management */
  document.getElementById('btnAddTeamMember')?.addEventListener('click', () => openUserForm());

  /* Sign out (replaces Switch User for Supabase) */
  ...
  /* Legacy login — Add user button */
  document.getElementById('btnAddUserLogin')?.addEventListener('click', () => { hideLoginScreen(); openUserForm(); });

  /* Data */
  clone('btnClearData').addEventListener('click', async () => {
    if (!confirm('Delete ALL your receipts? This cannot be undone.')) return;
    await Api.clear();
  ...
  cz.addEventListener('drop', e => { e.preventDefault(); cz.style.opacity=''; handleFileInput(e.dataTransfer.files[0]); });
}
...
document.addEventListener('DOMContentLoaded', () => {
  /* Set up auth listeners immediately (before init) */
  if (USE_SUPABASE) {
    setupAuthListeners();
  } else {
    /* IndexedDB pre-login: "Add Profile" from login screen */
    document.getElementById('btnAddUserLogin')?.addEventListener('click', () => {
      DB.open().then(() => DB.getAllUsers()).then(users => {
        State.allUsers = users; hideLoginScreen(); openUserForm();
      });
    });
    document.getElementById('confirmUserForm')?.addEventListener('click', async () => {
      ...
      try {
        if (editId) { userData.id=Number(editId); await DB.updateUser(userData); } else { const id=await DB.addUser(userData); userData.id=id; }
        State.allUsers = await DB.getAllUsers();
        document.getElementById('userFormModal').classList.add('hidden');
        if (!State.currentUser) { _showLoginScreen(); } else renderUserManagementList();
